...
namespace AbotEsge.Util
{
    ...
    /// <summary>
    /// Bloom filter.
    /// more info on https://llimllib.github.io/bloomfilter-tutorial/
    /// </summary>
    ...
    public class BloomFilter<T> : IBloomFilter<T>
    {
        ...
        public void Save()
        {
            ...
            using (var s = new FileStream("bloomFilter.bin", FileMode.Create))
            {
                formatter.Serialize(s, _hashBits);
                s.Flush();
            }
        }
        public void Resume()
        {
            ...
            using (var s = new FileStream("bloomFilter.bin", FileMode.Create))
            {
                var array = formatter.Deserialize(s) as BitArray;
        ...
        /// <returns></returns>
        private static int HashIp(T input)
        {
            var ip = input as IPAddress;
            var s = ip.GetArpaName();
            var hash = 0;
